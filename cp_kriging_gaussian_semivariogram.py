...
# -*- coding: utf-8 -*-
"""
Pump Efficiency Analysis with B-spline + Gaussian Kriging using Semivariogram
...
def ordinary_kriging_1d(x_data, y_data, x_pred, nugget, sill, vrange, variogram_func):
    n = len(x_data)
    cov_matrix = np.array([[sill - variogram_func(abs(xi - xj), 0.0, sill, vrange) for xj in x_data] for xi in x_data])
...
if __name__ == "__main__":
    ...
    logger.info("=== Semivariogram Estimated Parameters ===")
    ...
    for n_pts, color in zip(points_list, colors):
        ...
        eta_pred = ordinary_kriging_1d(q_spline, eta_spline, q_dense,
    ...
    plt.title("Pump Efficiency: Cubic Spline + Gaussian Kriging (Semivariogram)")
    plt.ylim(0, 100)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
